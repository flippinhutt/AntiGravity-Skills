...
`}}}getTemplates(e){return[this.getMinimalTemplate(e),this.getScriptBasedTemplate(e),this.getMultiAgentTemplate(e)]}async generateTemplate(e,n,s){let t=this.getTemplates(e).find(i=>i.name===n);if(!t)throw new Error(`Template "${n}" not found.`);f.existsSync(s)||f.mkdirSync(s,{recursive:!0});for(let[i,a]of Object.entries(t.files)){let l=C.join(s,i),c=C.dirname(l);f.existsSync(c)||f.mkdirSync(c,{recursive:!0}),f.writeFileSync(l,a)}}};function U(d){console.log("Antigravity Skill Manager is now active!");let e=new I(d.globalState),n=new S(d.extensionUri,"local",e);d.subscriptions.push(o.window.registerWebviewViewProvider("antigravity.localSkills",n));let s=new S(d.extensionUri,"remote",e);d.subscriptions.push(o.window.registerWebviewViewProvider("antigravity.remoteSkills",s));let r=new class{async provideTextDocumentContent(t){try{let i=await fetch(t.query);if(!i.ok)throw new Error(`HTTP ${i.status}: ${i.statusText}`);return await i.text()}catch(i){return`Failed to fetch remote file content:
